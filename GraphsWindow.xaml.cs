...
namespace GreenResourceMonitor
{
	...
	public partial class GraphsWindow : Window
	{
		...
		private void LoadCsv()
		{
			...
			{
				MessageBox.Show("No snapshot data found.", "Snapshot Error", MessageBoxButton.OK, MessageBoxImage.Error);
				return;
			}
			...
			{
				MessageBox.Show($"Error loading CSV: {ex.Message}", "CSV Error", MessageBoxButton.OK, MessageBoxImage.Error);
			}
		}
		...
		private void PlotData(string process, string metrics)
		{
			...
			{
				MessageBox.Show($"End date cannot be earlier than Start date.", "Time inconsistency");
			...
			{
				MessageBox.Show("No data available for the selected filters.", "No data", MessageBoxButton.OK, MessageBoxImage.Error);
				PlotView.Refresh();
				return;
			}

			// The issue arises because the CPU usage percentage is being summed across all processes when "All" is selected.
			// This can lead to values exceeding 100%, which is not meaningful in this context.
			// To fix this, the CPU usage percentage should be averaged instead of summed when plotting for "All" processes.
		...
		private void ExportButton_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				{
					MessageBox.Show("No data available for the selected filters to export.", "No data", MessageBoxButton.OK);
					return;
				}
